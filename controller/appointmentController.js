...
export const postAppointment =catchAsyncError(async (req, res, next) => {
    const { firstName,lastName,  email, phone, nic,dob, gender, appointment_date, department,
...
res.status(200).json({succes:true,message:"Appointment Sent Successfully",appointment});
}) 
...
export const getAllAppointments = catchAsyncError(async (req, res, next) => {
  ...
  export const updateAppointmentStatus =catchAsyncError(async(req,res,next)=>{
    ...
    res.status(200).json({succes:true,message:"Appointment Status Updated",appointment});
})
